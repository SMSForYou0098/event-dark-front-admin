...
const AuditoriumTicketBooking = ({ layoutId, eventDetails }) => {
  ...
  const getSeatColor = (seat, sectionId, rowId) => {
    const seatIdentifier = `${sectionId}-${rowId}-${seat.id}`;
    const isSelected = selectedSeats.some(s => s.identifier === seatIdentifier);
  ...
  return (
    <Card className="auditorium-booking">
      {/* Top Toolbar */}
                      ...
                      {row.seats.map(seat => (
                        <Group key={seat.id}>
                          <Circle
                            ...
                            y={seat.y}
                            radius={seat.radius}
                            fill={getSeatColor(seat, section.id, row.id)}
                            stroke="#333"
                            strokeWidth={0.5}
                            onClick={() => handleSeatClick(seat, section.id, row.id)}
                            onTap={() => handleSeatClick(seat, section.id, row.id)}
          ...
          <Card title="Booking Summary" style={{ height: '100%' }}>
            ...
            <div className="legend" style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid #eee' }}>
              ...
              <Space direction="vertical" size={4}>
                <Space>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', backgroundColor: '#FF6B6B' }} />
                ...
                <Space>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', backgroundColor: '#FF9800' }} />
                  <span style={{ fontSize: 12 }}>Reserved</span>
                </Space>
                <Space>
                  <div style={{ width: 16, height: 16, borderRadius: '50%', backgroundColor: '#9E9E9E' }} />
      ...
      >
        ...
        <div style={{ marginTop: 16, padding: 12, background: '#f0f7ff', borderRadius: 4 }}>
          <p style={{ margin: 0, fontSize: 12, color: '#666' }}>
            <strong>Note:</strong> This is a demo booking. In production, your seats will be held for 10 minutes and you'll be redirected to payment.
          </p>
        </div>
      </Modal>
    </Card>
